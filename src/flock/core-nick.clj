...
(def fout (clojure.java.io/writer "simulator/evostats.txt"))
...
(defn evolve
  ...
  (if (<= n 0)
    population 
    (recur (next-generation population)
           (let[
           best-index (.indexOf (:fitness population)
                        (apply max (:fitness population)))
           best-fitness (nth (:fitness population) best-index)
           avg-fitness (apply avg (:fitness population))]
           (if fout
             (.write fout
               (str	(.toString n) ","
                    (.toString avg-fitness) ","
                    (.toString best-fitness) "\r\n")))
           (println "Inverse Generation" n)
